...
describe('TasksService', () => {
  ...
  describe('buildTaskWhere()', () => {
    ...
    it('adds status, priority, assignedTo and contactId filters', async () => {
      const { service } = makeService(prisma)
      const where = await service.buildTaskWhere(TENANT, USER, {
        status: 'IN_PROGRESS',
        priority: 'HIGH',
    ...
    it('normalises dueDateTo to 23:59:59.999', async () => {
      ...
      // Normalize the upper date boundary to the end of the requested day (AC 27).
  ...
  describe('auto-logging TASK_COMPLETED (Story 4.2)', () => {
    ...
    it('an orphan task (contactId null) completes but logs nothing (AC 21 / UT6)', async () => {
      const { service, activity } = makeService(prisma)
      prisma.task.findFirst
        .mockResolvedValueOnce(makeTask({ status: 'TODO', contactId: null }))
        .mockResolvedValueOnce(makeTask({ status: 'COMPLETED', contactId: null }))
      prisma.task.updateMany.mockResolvedValue({ count: 1 })
