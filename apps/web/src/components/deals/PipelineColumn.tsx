...
export function PipelineColumn({
  ...
  return (
    <div className="flex w-72 shrink-0 flex-col rounded-lg bg-slate-50">
      {/* Sticky header */}
      <div
        ref={setNodeRef}
        className="sticky top-0 z-10 rounded-t-lg border-b border-slate-200 bg-slate-50 px-3 py-3"
      >
        ...
        <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
          <span>
            <strong className="text-slate-700">{count}</strong> deals
          </span>
      ...
      >
        ...
        {deals.length === 0 && <p className="py-8 text-center text-xs text-slate-400">No deals</p>}
      </div>
    </div>
  )
}
