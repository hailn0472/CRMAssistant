...
type DealFormDrawerProps = {
  ...
  /** Present → editing this deal. Absent → creating a new one. */
  deal?: Deal
...
export function DealFormDrawer({
  ...
  deal,
  onSaved,
}: DealFormDrawerProps): React.JSX.Element | null {
  ...
  function handleSaved(savedDeal: Deal): void {
    void queryClient.invalidateQueries({ queryKey: ['deals'] })
    void queryClient.invalidateQueries({ queryKey: ['deals-metrics'] })
    if (deal) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
        ...
        <DealForm deal={deal} onSaved={handleSaved} onCancel={() => onOpenChange(false)} />
      </div>
    </div>,
    document.body,
  )
