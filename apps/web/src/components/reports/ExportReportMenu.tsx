...
export function ExportReportMenu({
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: async (exportRecord: ReportExport, format: ReportDeliveryFormat) => {
  ...
  const handleExport = (format: ReportDeliveryFormat) => {
    if (exportMutation.isPending || disabled || !reportId) return
  ...
  const triggerButton = (
    <Button
      type="button"
      variant="outline"
      disabled={disabled || !reportId || isExporting}
      ...
      >
        <div role="menu" aria-label="Export formats" className="flex flex-col gap-0.5">
          ...
          >
            ...
            <div className="flex flex-col">
              <span>CSV Plain</span>
              <span className="text-[11px] font-normal text-[#8c8c96]">Raw data with headers</span>
            </div>
          </button>
        </div>
      </PopoverContent>
