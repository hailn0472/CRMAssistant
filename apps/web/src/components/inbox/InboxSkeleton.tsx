      ...
      >
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className={cn('flex w-full', i % 2 === 0 ? 'justify-start' : 'justify-end')}>
            <div
              className={cn(
                'h-16 w-64 rounded-2xl bg-[#ececf0] shadow-sm',
    ...
    <div className={cn('flex flex-col w-full animate-pulse gap-1', className)}>
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="flex items-start gap-3 p-3">
          <div className="h-12 w-12 flex-shrink-0 rounded-full bg-[#ececf0]" />
          <div className="flex flex-1 flex-col gap-2 pt-1">
            <div className="flex items-center justify-between">
              <div className="h-4 w-32 rounded bg-[#ececf0]" />
              <div className="h-3 w-12 rounded bg-[#ececf0]" />
            </div>
            <div className="h-3 w-3/4 rounded bg-[#ececf0]" />
          </div>
        </div>
      ))}
    </div>
