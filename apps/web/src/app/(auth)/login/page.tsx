...
function LoginForm(): React.JSX.Element {
  ...
  const handle2FASubmit = useCallback(async (code: string): Promise<void> => {
    if (!tempToken) return
      ...
      <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-8 text-slate-950">
        <Card className="w-full max-w-md border-slate-200 bg-white shadow-sm">
          <CardHeader className="space-y-2 text-center">
            <CardTitle className="text-2xl font-semibold tracking-tight" role="heading" aria-level={1}>
              Xác thực hai yếu tố
            </CardTitle>
...
function Enable2FASetupWizard({ onComplete }: { onComplete: () => void }): React.JSX.Element | null {
