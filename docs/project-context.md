...
- **Known limitation**: pub/sub is an in-process `EventEmitter` (`apps/api/src/inbox/pubsub.service.ts`, `apps/api/src/deals/deal-pubsub.service.ts`, `apps/api/src/tasks/task-pubsub.service.ts`, Story 4.4's sibling `apps/api/src/activities/activity-pubsub.service.ts`, and Story 4.8's sibling `apps/api/src/notifications/notification-pubsub.service.ts`) — **single-instance only**, and events emitted with no pending consumer are dropped (fan-out, not a job queue). Multi-instance deployment requires Redis-backed pub/sub first.
...
- **Application metrics [SHIPPED]**: the NestJS API exposes Prometheus-compatible `/metrics`, `/health/live`, and `/health/ready`; HTTP, GraphQL, Prisma, cache, Facebook, dependency, and background-job paths use a shared bounded metric catalog. Repository-owned Grafana dashboard, alert rules, and runbooks live in `infra/observability/`. Deployment activation is a separate rollout step: the free-tier integration is Grafana Cloud Metrics Endpoint scraping the public Render URL every 60 seconds with a Bearer token; no Render Metrics Stream, Alloy service, or self-hosted Prometheus is assumed. Render Free is treated as hobby/demo/staging, not as a production availability tier.
...
**Frontend** — `apps/web/src/`: `app/(auth)`, `app/(dashboard)/<domain>` (Story 4.4 added `app/(dashboard)/activities` — the List/Calendar/Timeline workspace route; Story 4.5 added `app/(dashboard)/reports/productivity`; Story 6.2 added `app/(dashboard)/reports` → redirect to `app/(dashboard)/reports/sales` — the `SalesReportsWorkspace` with saved reports, filters, comparison, chart and drill-down; Story 6.3 added `app/(dashboard)/reports/builder` — the thin `/reports/builder` page rendering `CustomReportBuilder` with Data Source / Fields / Visualization sections, `@dnd-kit` drag-and-drop plus keyboard/button movement, 300 ms debounced `customReportPreview`, React Hook Form + Zod save dialog and `?reportId=` edit mode; Story 6.5 added `app/(dashboard)/reports/schedules` — the `ReportSchedulesPage` workspace for scheduled email delivery; Story 6.6 added `app/(dashboard)/reports/exports` — the `ReportExportsPage` workspace for downloading asynchronous export artifacts; Story 6.7 added `app/(dashboard)/reports/customer-analytics` — the thin route and `CustomerAnalyticsPage` dashboard for CLV, churn risk distribution, 90-day LTV trends, and cohort analysis), `app/api/graphql` (proxy route), `components/<domain>`, `components/ui` (shadcn), `components/shared`, `components/layout`, `services/<domain>.service.ts`, `lib/`, `hooks/`, `stores/`.
...
- **Story 4.7 model**: `Note` (per-user authored prose on Contact or Deal — one nullable `contactId` FK and one nullable `dealId` FK, exactly-one-set enforced in `NotesService` because Prisma cannot express a cross-column check constraint; `onDelete: Cascade` on both parent FKs, `onDelete: Restrict` on `author`; soft-deleted with `deletedAt`; no `@@unique`; **no `accountId`** — Account is deferred per `architecture.md:922-929`). 🚨 `Contact` already has a scalar `notes String?` column (`:374`); the `Note[]` back-relation on `Contact` is therefore named `noteEntries` to avoid a `prisma generate` collision. `Contact` now carries both a free-text `notes` column AND a `noteEntries Note[]` relation — they are semantically distinct (the free-text field is an unversioned blob; `Note` rows are individually authored, editable, and surfaced on the timeline).
- **Story 4.8 model**: `Notification` (recipient-scoped in-app notification with three server-side producers — `TASK_ASSIGNED`, `DEAL_REMINDER`, `DEAL_MENTION`; type is a `String` validated against a const tuple in the service, not a Prisma enum; at most one of `dealId`/`taskId` non-null; `dedupeKey` is unique-when-set via `@@unique([tenantId, dedupeKey])` where NULL keys never collide because Postgres treats NULLs as distinct in unique indexes; `onDelete: Cascade` on all four relations — tenant, recipient (not Restrict as Note.author; a notification is worthless without its recipient), deal and task; soft-deleted with `deletedAt`; no createNotification/deleteNotification mutations; no audit rows (derived from already-audited mutations — the Activity carve-out); `@@index([tenantId, userId, createdAt(sort: Desc)])` serves the list query, `@@index([tenantId, userId, readAt])` serves the unread count; own-scoped subscription channel with no visibility filter — recipient-scoping is strictly stronger).
- **Story 6.2 model**: `Report` (tenant-scoped saved sales report — `type String` validated against the `REPORT_TYPES` const tuple, `config Json` typed-config pattern exactly like `Widget.config` (strict `validateReportConfig` on write, defensive `parseReportConfig` on read — no JSON GraphQL scalar), **no `@@unique`** on name (active-name uniqueness per `(tenantId, createdBy)` enforced in `SalesReportsService` so soft-deleted names can be reused), four indexes `@@index([tenantId])` / `@@index([tenantId, createdBy])` / `@@index([tenantId, type])` / `@@index([tenantId, isPublic])`, soft-deleted with `deletedAt`; `isPublic` means tenant-wide read/run only — it never grants update/delete and never expands deal visibility).
- **Story 6.3 (as-built report map)**: no model/migration — the builder persists into the existing `Report` row with `type='CUSTOM'` and a versioned `CustomReportConfig` JSON (`version: 1`, `dataSource`, `filters`, `dimensions`, `metrics`, `calculatedFields`, `visualization`, `sort`). Backend: `apps/api/src/reports/custom-report-types.ts` (closed vocabularies), `custom-report-config.ts` (strict `validateCustomReportConfig` / defensive `parseCustomReportConfig` + safe arithmetic expression parser — no `eval`/`new Function`/raw SQL), `custom-report-catalog.ts` (server-owned per-source field catalogue), `custom-reports.service.ts` (four-source aggregation engine sharing `buildContactWhere`/`buildDealWhere`/`buildTaskWhere`/`buildFeedWhere` visibility predicates), and the typed Pothos surface in `reports.graphql.ts` (`customReportFieldCatalog`, `customReportPreview`, `customReportData`, `saveCustomReport`). Frontend: `apps/web/src/app/(dashboard)/reports/builder/page.tsx` (thin page; no second QueryProvider), `components/reports/CustomReportBuilder.tsx` (orchestrator: `['customReports']`-rooted TanStack keys, 300 ms debounced + cancelled preview that never requests invalid drafts, RHF+Zod save dialog, `?reportId=` edit mode), `CustomReportDataSourcePanel.tsx` (four entity cards + source-change confirmation + filter rows + always-visible active-filter summary), `CustomReportFieldsPanel.tsx` (@dnd-kit PointerSensor 8px/Touch/Keyboard + DragOverlay + announcements, Add/Remove/Move up/down equivalents, calculated-field editor), `CustomReportVisualizationPanel.tsx` (5 chart choices + display options + inline compatibility errors), `CustomReportPreview.tsx` (validation/loading/refreshing/error/empty/data states; Recharts line/bar/pie/funnel + semantic table + `role="img"` + legend + tooltip + complete sr-only table), `lib/custom-report-builder.ts` (pure draft reducer/validation mirror), `services/custom-report.service.ts` (exact operations/variables/unwraps). The `/reports/sales` route stays intact and unaffected.
- **Story 6.4 (as-built chart framework & drill-down map)**: no new models/migrations or dependencies. Introduces `CustomReportConfig` version 2 with expanded chart types (`TABLE|LINE|BAR|PIE|DONUT|AREA|FUNNEL|SCATTER|HEATMAP`), closed `CustomReportColorToken` (10 tokens mapped to approved palette hexes) and `CustomReportLegendPosition` (`TOP|RIGHT|BOTTOM|LEFT`). Stable series point keys (`key`) and ordered `dimensionLabels` support bounded complete chart series. Reusable `ReportChart` framework (`apps/web/src/components/reports/charting/ReportChart.tsx`) with accessible theme tokens (`report-chart-theme.ts`), viewport controls (Zoom/Pan/Reset with 44px touch targets and polite range live-region), static selector previews (`ReportChartTypePreview.tsx`), client-side PNG/SVG export (`lib/report-chart-export.ts` with 2x canvas rasterization and Blob cleanup), and tenant-safe drill-down (`customReportDrillDown` query + `CustomReportDrillDownSheet.tsx`). Consolidates sales report chart and dashboard widgets.
- **Story 6.6 (as-built report export map)**: Model `ReportExport` with `ReportExportStatus` (`PENDING`, `PROCESSING`, `READY`, `FAILED`), format `ReportDeliveryFormat` (`PDF`, `EXCEL`, `CSV`), relations to `Tenant`, `Report`, `User`, and `Notification.reportExportId` (mutually exclusive with deal/task targets). Private Supabase Storage bucket (`REPORT_EXPORT_STORAGE_BUCKET` default `report-exports`) with tenant-first paths `reports/<tenantId>/<userId>/<exportId>/<filename>` and fresh 24-hour (86,400s) signed URLs minted on demand via `reportExportDownloadUrl`. Server-side pure SVG chart generation + `@resvg/resvg-js` rasterization embedded in PDFKit numbered PDFs and ExcelJS 3-sheet (`Summary`, `Data`, `Charts`) workbooks with formula AST preservation and formula injection neutralization. Durable PostgreSQL-backed minute worker `ReportExportProcessor` with lease/stale-claim recovery for custom exports > 100 rows; small sales/custom exports generate inline. Frontend: `ExportReportMenu` on saved sales and custom builder surfaces, thin route `/reports/exports` + `ReportExportsPage` with responsive cards, 320px viewport compliance, pagination, filtering, delete modal, and guarded `?download=<id>` deep link. `ReportExport.filters` is the immutable server-normalized effective config snapshot — sales persist the FULL validated effective config (saved config merged with validated request overrides, never raw GraphQL input), CUSTOM persists null (saved config owns filters) — so background replay reproduces the exact effective scope even if the saved config changes mid-flight. USER_EXPORT filenames embed sanitized deterministic filter tokens (`owner-<id8>`/`team-`/`stage-`/`product-` for sales scoping, `filters-<sha8>` for custom non-date filter sets) plus the effective date range or `as-of_YYYY-MM-DD`, while the full human-readable filter summary stays in document metadata. Optional operational override `REPORT_EXPORT_MAX_BYTES` allows adjusting the 50 MB (52,428,800 bytes) ceiling.
- **Story 6.7 (as-built customer analytics map)**: Models: `Contact` extended with nullable server-owned fields (`lifetimeValue`, `churnRisk`, `churnRiskScore`, `lastActivityDate`, `analyticsCalculatedAt`) + 3 tenant indexes; daily snapshot model `CustomerAnalyticsSnapshot` with unique `(tenantId, contactId, snapshotDate)` and 5 indexes; `Task` internal-only automation fields `automationSource` and `automationKey` with `@@unique([tenantId, automationKey])`. Pure scoring in `customer-analytics-score.ts`: LTV = sum of closed-won deals (`DealStage.isWon=true`), inactivity horizon 90 UTC days (40%), win-rate risk (30%, neutral 50 for no-closed), engagement score across 5 qualifying activity types (30%), category boundaries (`LOW` < 30, `MEDIUM` 30–70, `HIGH` > 70), recommended actions (`SCHEDULE_FOLLOW_UP`, `UPSELL_OPPORTUNITY` for LOW + top 25% LTV, `MONITOR`), and mixed-currency breakdown without false currency symbol labels or FX assumptions. Background daily job `CustomerAnalyticsProcessor` (`0 0 2 * * *`) processes tenant and contact batches (default 500) with 4 aggregate groupBys (no N+1), short transactions, bounded P2034 retries, and creates idempotent follow-up tasks on `HIGH` transition assigned to the contact owner. Backend GraphQL `customerAnalytics(filters, pagination)` gated on `REPORT:READ` + `CONTACT:READ` + `DEAL:READ` with shared visibility from `ContactsService.buildContactWhere`. Frontend: thin route `app/(dashboard)/reports/customer-analytics/page.tsx`, `CustomerAnalyticsPage.tsx` dashboard with 4 metric cards (Total LTV, Average LTV, analyzed/total count, High-Risk count), persistent mixed-currency warning banner, 4 `ReportChart` analytical charts with always-present sr-only semantic tables (`hideSrTable=false` default, no visible chart/table toggles: LTV distribution, Churn risk donut, 90-day LTV trend, acquisition cohort analysis), filter toolbar with inline error validation, and responsive customer list table with exact badges, links to `/contacts/<id>`, and pagination. Query keys rooted at `['customerAnalytics']`. Test surface map (Contract F53): pure score spec (`apps/api/src/reports/__tests__/customer-analytics-score.spec.ts`), processor spec (`apps/api/src/reports/__tests__/customer-analytics-processor.service.spec.ts`), service/GraphQL specs (`apps/api/src/reports/__tests__/customer-analytics.service.spec.ts`, `customer-analytics.graphql.spec.ts`), tasks automation spec (`apps/api/src/tasks/__tests__/tasks-automation.spec.ts`), Testcontainers customer analytics integration (`apps/api/test/integration/customer-analytics.integration.spec.ts`), frontend service/lib/RTL/route/nav/breadcrumb specs (`apps/web/src/services/__tests__/customer-analytics.service.spec.ts`, `apps/web/src/lib/__tests__/customer-analytics.spec.ts`, `apps/web/src/components/reports/__tests__/CustomerAnalyticsPage.spec.tsx`, `apps/web/src/app/(dashboard)/reports/__tests__/customer-analytics-page.spec.tsx`, `apps/web/src/components/layout/__tests__/AppShellNavigation.spec.tsx`, `Breadcrumbs.spec.tsx`); Playwright E2E (`tests/e2e/customer-analytics.spec.ts`) and dogfood QA pending Stage 9 while status remains in-progress.
