...
- Always include a body for feature, fix, refactor, test, ci, build, and chore commits
...
- Embed screenshots directly for UI-facing changes.
- Include desktop, tablet, and mobile screenshots when practical.
- If screenshots cannot be captured, document the blocker and retry command/env.
...
### PR Screenshot Requirements

For PRs that include user-facing UI changes, always attach or embed screenshots
in the `## Screenshots (if applicable)` section before opening the PR when the
app can be run locally. Capture desktop, tablet, and mobile viewports when
practical.

Preferred workflow:

1. Save screenshots in `docs/pr-screenshots/<story-key>/`.
2. Commit the screenshot files with the branch.
3. Embed them in the PR body using markdown image syntax with GitHub blob URLs
   and `?raw=true` so reviewers can view them inline.
4. Add direct links to the screenshot files as a fallback.

If screenshots cannot be captured or embedded, keep the section in the PR body
and document the blocker plus the command/env needed to retry.
...
- Feature/story branches must be integrated through a pull request. Do not merge, squash, rebase, cherry-pick, or commit feature work directly into `dev` unless the user explicitly requests a direct local-only operation.
- When the user asks to “merge feature branch into dev”, interpret that as creating a PR from the feature branch to `dev`, not performing a direct merge into `dev`.
