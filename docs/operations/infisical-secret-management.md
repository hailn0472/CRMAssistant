...
| `SUPABASE_STORAGE_BUCKET`        | API          | dev, staging, production | `/apps/api`                                 | internal           | Supabase                   | Render backend only                              | Must match the private bucket created in the Supabase project                |
...
| `ENCRYPTION_KEY`                 | API/security | dev, staging, production | `/apps/api`                                 | critical           | Generated secret           | Render backend, local API dev                    | Rotate on incident; changing it invalidates all previously encrypted tokens |
| `FACEBOOK_APP_ID`                | API/platform | dev, staging, production | `/apps/api`                                 | internal           | Meta for Developers app    | Render backend, local API dev                    | Update when the Facebook app changes                                        |
| `FACEBOOK_APP_SECRET`            | API/security | dev, staging, production | `/apps/api`                                 | critical           | Meta for Developers app    | Render backend only                              | Rotate after migration and on incident; backend-only                        |
| `FACEBOOK_VERIFY_TOKEN`          | API/security | dev, staging, production | `/apps/api`                                 | internal           | Generated secret           | Render backend, local API dev                    | Rotate if leaked; must match the webhook subscription config                |
| `FACEBOOK_GRAPH_API_VERSION`     | API          | dev, staging, production | `/apps/api`                                 | internal           | App config                 | Render backend, local API dev                    | Update when pinning a newer Graph API version                               |
| `GOOGLE_CALENDAR_CLIENT_ID`      | API          | dev, staging, production | `/apps/api`                                 | internal           | Google Cloud OAuth app     | Render backend, local API dev                    | Update when the Google Cloud OAuth client changes                           |
| `GOOGLE_CALENDAR_CLIENT_SECRET`  | API/security | dev, staging, production | `/apps/api`                                 | critical           | Google Cloud OAuth app     | Render backend only                              | Rotate after migration and on incident; backend-only                        |
| `GOOGLE_AI_STUDIO_API_KEY`       | API/security | dev, staging, production | `/apps/api`                                 | critical           | Google AI Studio           | Text-to-SQL provider in Render backend and local API dev | Rotate immediately if exposed; never expose to the frontend                 |
| `GOOGLE_AI_STUDIO_MODEL`         | API          | dev, staging, production | `/apps/api`                                 | internal           | App config                 | Text-to-SQL provider in Render backend and local API dev | Pin an approved Gemini model; defaults to `gemini-3.6-flash` when unset     |
| `MICROSOFT_CALENDAR_CLIENT_ID`   | API          | dev, staging, production | `/apps/api`                                 | internal           | Azure AD app registration  | Render backend, local API dev                    | Update when the Azure AD app changes                                        |
| `MICROSOFT_CALENDAR_CLIENT_SECRET` | API/security | dev, staging, production | `/apps/api`                               | critical           | Azure AD app registration  | Render backend only                              | Rotate after migration and on incident; backend-only                        |
...
High-risk variables are `DATABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `JWT_SECRET`, `GOOGLE_AI_STUDIO_API_KEY`, `VERCEL_TOKEN`, `RENDER_DEPLOY_HOOK_URL`, `RENDER_DEPLOY_HOOK_URL_STAGING`, and Render API keys or deploy hooks used for platform connections. Treat Infisical machine identity secrets as high-risk bootstrap secrets.
